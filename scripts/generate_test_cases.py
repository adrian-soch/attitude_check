...
def get_marg_estimate_cases(freq):
    ...
    est = Madgwick(gyr=gyr, acc=acc, mag=mag, frequency=freq,
                   q0=q0, gain_imu=0.033, gain_marg=0.041)
...
def get_imu_estimate_cases(freq):
    ...
    acc = np.array([[0.47451228800383594, 1.0183972122654374, 8.461165125378162],
                    ...
                    [0.27340728800383585, 0.05015021226543746, 8.015791125378161],
                    [0.3067612880038358, -0.2853517877345626, 8.159017125378163]], dtype=np.float32)
    est = Madgwick(gyr=gyr, acc=acc, frequency=freq,
                   q0=q0, gain_imu=0.033, gain_marg=0.041)
...
def main():
    ...
    get_marg_estimate_cases(freq=285.71428571)
    get_imu_estimate_cases(freq=285.71428571)
